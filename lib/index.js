...
app.post('/api/v1/kraken', function (req, res) {
  (0, _child_process.execFile)(process.env.CRYPTOCHECK_PATH, ['kraken', parseCurrency(req.body.text)], function (err, stdout) {
...
app.post('/api/v1/gdax', function (req, res) {
  (0, _child_process.execFile)(process.env.CRYPTOCHECK_PATH, ['gdax', parseCurrency(req.body.text)], function (err, stdout) {
    res.json({ text: stdout });
  });
});
...
app.post('/api/v1/cmc', function (req, res) {
  (0, _child_process.execFile)(process.env.CRYPTOCHECK_PATH, ['cmc', parseCurrency(req.body.text)], function (err, stdout) {
    res.json({ text: stdout });
  });
});
